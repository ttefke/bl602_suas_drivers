...
int suas_adc_init(suas_adc_conf_t *conf) {
  ...
  if (conf->conversion_mode == NORMAL_ADC_CONVERSION_MODE) {
    if ((conf->frequency < 40) || (conf->frequency > 1400)) {
      printf(
          "Selected frequency does not match for normal ADC conversion "
          "mode!\r\n");
      return -1;
    }
    ...
    if ((conf->frequency < 500) || (conf->frequency > 16000)) {
      printf(
          "Selected frequency does not match for microphone ADC conversion "
          "mode!\r\n");
      return -1;
    }
  }
  ...
  bl_adc_freq_init(conf->conversion_mode,
                   conf->frequency);  // always returns 0 -> nothing to evaluate

  // Initialize GPIO pin for single channel conversion mode
  bl_adc_init(conf->conversion_mode,
              conf->pin);  // always returns 0 -> nothing to evaluate
  ...
  int channel = bl_adc_get_channel_by_gpio(
      conf->pin);  // returns -1 if channel does not exist but we already tested
                   // for that
...
uint32_t suas_adc_read(suas_adc_conf_t *conf) {
  ...
  memcpy((uint8_t *)adc_data,            /* destination: adc_data array */
         (uint8_t *)(ctx->channel_data), /* source: channel_data element of ctx
                                            struct */
